...
//    }
  ...
  /** Some other code for further enhancement **/
//  def constructPatientBasedOnMortalityPeriod(spark: SparkSession, patient: Dataset[Patient],
//                                             icustays: Dataset[IcuStays], chart: Dataset[Chart],
//                                             mortType: MortalityType) : RDD[Patient] ={
//
//    val MILLISECONDS_IN_DAY = 24L * 60 * 60 * 1000
//    val MILLISECONDS_IN_YEAR = MILLISECONDS_IN_DAY * 365L
//
...
//    val scICUStaysMap = spark.sparkContext.broadcast(icuStaysMap)
//
//    //val scChartMap = spark.sparkContext.broadcast(chart.rdd.map(x => (x.patientID, x.lastChartDate)).collectAsMap())
//
//    mortType match {
//      case InICU12Hr() => {
//        patient.rdd
...
//            if (scICUStaysMap.value.contains(x.patientID)) {
//              val icuData = scICUStaysMap.value(x.patientID)
//              val endTime12Hr = icuData.inDate.getTime + 12L * 3600 * 1000
//              val endTime = if (endTime12Hr < icuData.outDate.getTime) endTime12Hr else icuData.outDate.getTime
//              if (x.isDead != 0 &&
//                x.dod.getTime >= icuData.inDate.getTime && x.dod.getTime <= endTime)
//                Patient(x.patientID, x.isMale, x.dob, 1.0, x.dod, x.dod,
//                  math.abs(x.dod.getTime - x.dob.getTime) / MILLISECONDS_IN_YEAR)
//              else
//                Patient(x.patientID, x.isMale, x.dob, 0.0, x.dod, new Date(endTime),
//                  math.abs(endTime - x.dob.getTime) / MILLISECONDS_IN_YEAR)
//            }
//            else
//              Patient(x.patientID, x.isMale, x.dob, 0.0, x.dod, x.indexDate,
//                math.abs(x.indexDate.getTime - x.dob.getTime)/MILLISECONDS_IN_YEAR)
...
//            if (scICUStaysMap.value.contains(x.patientID)) {
//              val icuData = scICUStaysMap.value(x.patientID)
//              val endTime24Hr = icuData.inDate.getTime + 24L * 3600 * 1000
//              val endTime = if (endTime24Hr < icuData.outDate.getTime) endTime24Hr else icuData.outDate.getTime
//              if (x.isDead != 0 &&
//                x.dod.getTime >= icuData.inDate.getTime && x.dod.getTime <= endTime)
//                Patient(x.patientID, x.isMale, x.dob, 1.0, x.dod, x.dod,
//                  math.abs(x.dod.getTime - x.dob.getTime) / MILLISECONDS_IN_YEAR)
...
//            if (scICUStaysMap.value.contains(x.patientID)) {
//              val icuData = scICUStaysMap.value(x.patientID)
//              val endTime48Hr = icuData.inDate.getTime + 48L * 3600 * 1000
//              val endTime = if (endTime48Hr < icuData.outDate.getTime) endTime48Hr else icuData.outDate.getTime
//              if (x.isDead != 0 &&
//                x.dod.getTime >= icuData.inDate.getTime && x.dod.getTime <= endTime)
//                Patient(x.patientID, x.isMale, x.dob, 1.0, x.dod, x.dod,
//                  math.abs(x.dod.getTime - x.dob.getTime) / MILLISECONDS_IN_YEAR)
...
//              val icuData = scICUStaysMap.value(x.patientID)
//              if (x.isDead != 0 &&
//                x.dod.getTime >= icuData.inDate.getTime && x.dod.getTime <= icuData.outDate.getTime)
//                Patient(x.patientID, x.isMale, x.dob, 1.0, x.dod, x.dod,
//                  math.abs(x.dod.getTime - x.dob.getTime) / MILLISECONDS_IN_YEAR)
//              else
//                Patient(x.patientID, x.isMale, x.dob, 0.0, x.dod, icuData.outDate,
//                  math.abs(icuData.outDate.getTime - x.dob.getTime) / MILLISECONDS_IN_YEAR)
//            }
//            else
//              Patient(x.patientID, x.isMale, x.dob, 0.0, x.dod, x.indexDate,
//                math.abs(x.indexDate.getTime - x.dob.getTime)/MILLISECONDS_IN_YEAR)
...
//              val icuData = scICUStaysMap.value(x.patientID)
//              if (x.isDead != 0 &&
//                x.dod.getTime > icuData.outDate.getTime &&
//                (math.abs(x.dod.getTime - icuData.outDate.getTime) / MILLISECONDS_IN_DAY) <= 30)
//                Patient(x.patientID, x.isMale, x.dob, 1.0, x.dod, x.dod,
//                  math.abs(x.dod.getTime - x.dob.getTime) / MILLISECONDS_IN_YEAR)
//              else
//                Patient(x.patientID, x.isMale, x.dob, 0.0, x.dod, new Date(icuData.outDate.getTime + 30 * MILLISECONDS_IN_DAY),
//                  math.abs((icuData.outDate.getTime + 30 * MILLISECONDS_IN_DAY) - x.dob.getTime) / MILLISECONDS_IN_YEAR)
//            }
//            else
//              Patient(x.patientID, x.isMale, x.dob, 0.0, x.dod, x.indexDate,
//                math.abs(x.indexDate.getTime - x.dob.getTime)/MILLISECONDS_IN_YEAR)
...
//                  math.abs(x.dod.getTime - x.dob.getTime) / MILLISECONDS_IN_YEAR)
//              else
//                Patient(x.patientID, x.isMale, x.dob, 0.0, x.dod, new Date(icuData.outDate.getTime + 356 * MILLISECONDS_IN_DAY),
//                  math.abs((icuData.outDate.getTime + 365 * MILLISECONDS_IN_DAY) - x.dob.getTime) / MILLISECONDS_IN_YEAR)
//            }
//            else
//              Patient(x.patientID, x.isMale, x.dob, 0.0, x.dod, x.indexDate,
//                math.abs(x.indexDate.getTime - x.dob.getTime)/MILLISECONDS_IN_YEAR)
